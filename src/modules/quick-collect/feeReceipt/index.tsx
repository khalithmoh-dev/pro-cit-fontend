...
const FeeReceipt: React.FC<PropsIF> = ({ UpdateChallanDialogOpen, setConcessionDialogOpen, selectedId }) => {
    ...
    useEffect(() => {
        const fetchData = async () => {
            ...
            try {
                await getFeeReceipt(selectedId);
    ...
    const totalBalanceAmount = selectedFeeReceipt?.payment?.reduce((total: number, item: { paid_amount: any; }) => {
        return total + parseFloat(item?.paid_amount || 0);
    }, 0).toFixed(2);
                                                                            ...
                                                                            <tbody>
                                                                                ...
                                                                                {selectedFeeReceipt?.payment?.map((item: any, index: any) => (
                                                                                    <tr key={index}>
                                                                                        ...
                                                                                        <td>{item?.paid_amount}</td> {/* Display balance amount */}
                                                                                    </tr>
                                                                                ))}
                                                                            </tbody>
                                                                        </table>
