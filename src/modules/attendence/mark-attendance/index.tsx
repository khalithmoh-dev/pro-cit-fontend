...
const MarkAttendancePage: React.FC = () => {
  ...
  const { getPeriod, period } = useTimeTableStore();
  ...
  const handleSubmit = async () => {
    ...
    const res = await createAttendance(requestBody);
    if (res) navigate('/calendar/view');
  };

  const tableHead = ['SL NO.', 'STUDENT NAME', 'USN NUMBER', 'ADMISSION STATUS', 'PRESENT'];

  useEffect(() => {
    if (id) getPeriod(id.split('@')[0]);
  }, [id]);

  useEffect(() => {
    if (period?.students) {
      const initialStudents = period.students.map((student) => ({
        _id: student._id,
  ...
  return (
    <div className={style.container}>
      <TableControlBox tableName="Mark Attendance">
